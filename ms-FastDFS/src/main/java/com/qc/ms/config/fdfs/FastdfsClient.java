...
public class FastdfsClient {
    private String WINDOW_UPLOAD_PATH = "F:" + File.separator + "upload/";
...
//    private String trackers_pro = "120.25.223.121";// 正式环境
    private String trackers_pro = "120.27.92.110";// 正式环境
//    private String fileDom = "120.25.223.121:8888";// storage访问的域名地址（给前端用的） 8888是storage默认端口
    private String fileDom = "120.27.92.110:8888";// storage访问的域名地址（给前端用的） 8888是storage默认端口
...
//        ClientGlobal.init("fast_client.conf");
        ...
        if (activeProfiles.length > 0) {
            if ("dev".equals(activeProfiles[0])) {
                trackers_pro = "120.27.92.110";
//                trackers_pro = "120.25.223.121";
                fileDom = "http://120.27.92.110:8888";// 图片地址前缀
    ...
    public String uploadFile(MultipartFile f, Integer... kb) {// 是否需要压缩图片
        if (!f.isEmpty()) {
            if (f.getSize() > 0) {
                ...
                byte[] bytes;
                String file_ext_name = Objects.requireNonNull(f.getContentType()).split("/")[1];// 取文件后缀名称（这样做不是很正确）
                try {
